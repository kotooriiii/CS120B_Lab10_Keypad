 ...
 *
 *	Demo Link: https://www.youtube.com/watch?v=gGOMOzLQJR8
 */
...
unsigned char getKeyIgnoringAsterisk() {
...
*/
	...
	//NewCodeSM
	unsigned char newcode[4] = {0,0,0,0};
	unsigned char newCodeSize = 4;
	unsigned char newCodeIndex = 0x00;
	unsigned char newCodeConfirmIndex = 0x00;
	unsigned char isNewCodeSuccessful = 0x00;
	int newCodeTimer = 0;
	
	//Global period
	double globalPeriod = 1;
/*
...
enum NewCodeSM { NC_INIT, NC_WAIT, NC_READ_CHAR0, NC_WAIT_RELEASE0, NC_RELEASE0, NC_CONFIRM, NC_READ_CHAR1, NC_WAIT_RELEASE1, NC_RELEASE1, NC_CHANGE};  

int keypadListenerSMTick(int state);
int sequenceSMTick(int state);
int lockInsideSMTick(int state);
int songSMTick(int state);
int newCodeSMTick (int state);
...
int newCodeSMTick(int state)
{
	...
	unsigned char currentKeyCombo = getKeyIgnoringAsterisk();
	//Returns key being pressed. It is combo with asterisk.
	//If no key being pressed or more than 1 keys being pressed, returns '\0'.
	//If asterisk isn't being pressed, returns '\n'.
	...
	switch(state)
	{
		...
		if(currentKeyCombo == '\0' || (currentKeyCombo == '\n' && currentKey != '*') || currentKeyCombo == '#')
		{
			state = NC_WAIT;
		}
		else
		...
		{
			state = NC_WAIT_RELEASE0;
		}
		break;
		
		case NC_WAIT_RELEASE0:
		if((currentKeyCombo == '\n' && currentKey != '*') || currentKeyCombo == '#')
		{
			state = NC_WAIT;
		}		
		else if(currentKeyCombo == '\0')
		{
			state = NC_RELEASE0;
		}
		break; 
		...
		else if(newCodeIndex >= newCodeSize) //size for new pin code
		{
			state = NC_CONFIRM;
		}
		else if (currentKeyCombo != '\0')
		...
		case NC_CONFIRM:
		if(currentKeyCombo == '#') //entering code 
		{
			state = NC_WAIT;
		}
		else if(currentKeyCombo != '\n' || currentKey == '*') //asterisk is being pressed, reset
		{
//			state = NC_WAIT;
		}	
		else if(currentKey != '\0') //a key is being pressed
		{
			state = NC_READ_CHAR1;
		}
		break;
		...
		//	state = NC_WAIT;
		//}	
		else if(newCodeTimer >= 2*1000) //time is up
		{
			//passed 2000ms
			state = NC_WAIT;
		}
		else if(!isNewCodeSuccessful) //not new code
		{
			state = NC_WAIT;
		}
		else 
		{
			state = NC_WAIT_RELEASE1;
		}
		break; 
		
		case NC_WAIT_RELEASE1:
		if(currentKeyCombo == '#') //entering code 
		{
			state = NC_WAIT;
		}
		...
		else if(currentKey  == '\0') //released the char
		{
			state = NC_RELEASE1;
		}
		break; 
		...
		else if (newCodeConfirmIndex >= newCodeSize)
		{
			state = NC_CHANGE;
		}
		else if(currentKey  != '\0') //released the char
	...
	switch(state)
	{
		...
		case NC_WAIT:
		newCodeIndex = 0x00; //reinit code to empty.
		newCodeConfirmIndex = 0x00;
		break;
		
		case NC_READ_CHAR0:
		newcode[newCodeIndex++] = currentKeyCombo;
		...
		case NC_READ_CHAR1:
		newCodeTimer += globalPeriod;
		if(currentKey == newcode[newCodeConfirmIndex])
		{
			newCodeConfirmIndex++;
			isNewCodeSuccessful = 1;
		}
		else
		{
			isNewCodeSuccessful = 0;
		}
